...
Locator advisory has a model-neutral 2,000-token generation cap, shared by its core request boundary and live scripts. The allowance covers any hidden reasoning plus the final structured object; the JSON Schema remains five fields and limits the visible summary to 500 characters. Callers may configure a smaller allowance, but `Math.min` prevents them from exceeding the feature cap. The timeout remains 15 seconds, and timeout or truncation still fails closed without a larger-budget retry, model switch, or weaker output format. Pricing stays external, cost estimates remain approximate policy guards, and no model-specific token table is embedded. Free or otherwise constrained endpoints may still be slow or unreliable; capability-aware model planning remains future work.
