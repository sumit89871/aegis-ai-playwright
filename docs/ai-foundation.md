...
The adapter uses the built-in HTTP client surface through `fetch`, bearer authentication, JSON requests, and `AbortController`. Aegis retains the provider-neutral `maxOutputTokens` abstraction and maps it to OpenRouter's `max_tokens` request parameter without model-specific branching. Endpoint parameter support can vary; strict structured-output requests continue to use `provider.require_parameters: true` and never weaken their output contract to gain compatibility. Response metadata may independently report `native_finish_reason: "max_tokens"`.

The OpenRouter wire request uses `reasoning: { exclude: true }`: Aegis requests that private reasoning be excluded from the response without attempting to disable reasoning that a model requires internally. Such reasoning tokens may still contribute to aggregate token usage. Raw reasoning is never retained, reported, or substituted for missing final answer content. This is a static compatibility and privacy policy with no model-specific branch; capability-aware reasoning configuration remains future work. A strict-schema HTTP 400 or 422 is currently classified as `provider-schema-rejected` without parsing the provider error body, so a rejection of another required request parameter may share that safe category until provider diagnostics are refined separately.
...
Locator advisory has model-neutral 2,000-token and 30-second feature caps, shared by its core request boundary, comparison client, and live scripts. The generation allowance covers hidden reasoning plus the final structured object; the JSON Schema remains five fields and limits the visible summary to 500 characters. `Math.min` lets callers configure smaller token or timeout limits while preventing them from exceeding either feature cap. Timeout or truncation still fails closed without partial parsing, a larger-budget retry, model switch, or weaker output format. Verifier retries remain zero and comparison retries remain at most one.

Increasing the timeout does not itself increase provider cost; billing depends on tokens actually processed. A timed-out provider request may nevertheless incur usage, and local aggregate cost may exclude failures when the provider returns no usage metadata. Pricing stays external, estimates remain approximate policy guards, and no model-specific pricing or timeout table is embedded. Reliable current models are recommended for live comparison, but 30 seconds does not guarantee every endpoint will complete; capability-aware planning remains future work.
