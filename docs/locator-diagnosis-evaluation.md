...
Presentation is independent of metric calculation. Interactive terminals receive grouped rich sections and delayed staged progress, while `--plain`, CI, redirected output, narrow terminals, and `TERM=dumb` receive stable text without animation or ANSI. `--no-animation` keeps supported rich formatting without dynamic cursor updates. Both JSON modes remain JSON-only contracts, and the underlying numerators, denominators, thresholds, rankings, and safety decisions are unchanged.
