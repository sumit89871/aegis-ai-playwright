...
Presentation is independent of metric calculation. Interactive terminals receive bordered panels at 80+ columns or compact rich sections at 72–79 columns, plus delayed staged progress when work lasts at least 400 ms. Thinking-style brightness pulses are the rich default; a fast holdout may finish before they appear, which is expected. `--plain`, CI, redirected output, narrower terminals, and `TERM=dumb` receive stable text without animation or ANSI. `--no-animation` keeps supported rich formatting without dynamic cursor updates. Both JSON modes remain JSON-only contracts, and the underlying numerators, denominators, thresholds, rankings, and safety decisions are unchanged. `npm run cli:demo:progress` exercises presentation only and does not read evaluation artifacts.
