...
The generic `cli` exports provide terminal-capability detection, terminal-cell width measurement, ANSI-safe truncation/padding, responsive bordered or compact sections, semantic notices/errors, and an injectable delayed progress lifecycle. Rich panels begin at 80 columns, compact rich rendering covers 72–79 columns, plain rendering handles narrower or non-interactive output, and very wide output is capped at 140 columns. Emoji, Unicode, ANSI brightness, color, and animation are separate capabilities.

`thinking` is the default rich interactive progress style. It uses one stable symbol and the exact dim → normal → bright → normal → dim → normal sequence every 150 ms; ANSI blink is never used. Delayed start remains 400 ms. Windows Terminal auto-selects `💭`, with `↻`, `[~]`, rotating spinner, static, and no-progress fallbacks. Timer handles are unref'd and cleanup resets ANSI, clears the transient stderr line, and restores the cursor before permanent output. `renderLocatorBlindHoldoutTerminal` still receives only the aggregate DTO and elapsed duration, while JSON serialization and domain calculation remain separate. No external terminal dependency is required.
