...
The `locator-observations` exports import the existing sanitized locator report into an anonymous content-addressed record, validate a separate human-review record, and run blind holdout evaluation through the established diagnosis and metrics APIs. Review validation returns deterministic structured issues with stable codes, JSON field paths, safe actual values, controlled allowed values, and repair suggestions; the legacy throwing validator delegates to the same rules. Core performs no consumer-specific filesystem discovery and knows no application host. Thin repository scripts collect relative-path artifacts, prepare reviews, validate them, and write ignored reports. The default is deterministic-only, requires no key or network, cannot repair a review, and cannot apply a locator.
