...
The generic `cli` exports provide terminal-capability detection, bounded text wrapping, responsive bordered or compact sections, semantic notices/errors, ANSI stripping, and an injectable delayed progress lifecycle. Rich panels begin at 80 columns, compact rich rendering covers 72–79 columns, plain rendering handles narrower or non-interactive output, and very wide output is capped at 140 columns. They contain no locator or evaluation logic. `renderLocatorBlindHoldoutTerminal` is the first consumer: it receives the aggregate DTO and elapsed duration, while JSON serialization and domain calculation remain separate. The 400 ms delayed progress lifecycle writes transient animation only to interactive stderr. No external terminal dependency is required.
