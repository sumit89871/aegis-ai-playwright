...
The `locator-observations` exports import the existing sanitized locator report into an anonymous content-addressed record, validate a separate human-review record, and run blind holdout evaluation through the established diagnosis and metrics APIs. Core performs no consumer-specific filesystem discovery and knows no application host. Thin repository scripts collect relative-path artifacts, prepare reviews, validate them, and write ignored reports. The default is deterministic-only, requires no key or network, and cannot apply a locator.
