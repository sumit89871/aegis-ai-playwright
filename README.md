...
Deterministic-only is the comparison default; mock mode is offline and reproducible. Live mode reuses the existing OpenRouter adapter and requires explicit network consent plus `AEGIS_AI_ENABLED`, `AEGIS_AI_ALLOW_NETWORK_CALLS`, `OPENROUTER_API_KEY`, `AEGIS_AI_MODEL`, `AEGIS_AI_INPUT_COST_PER_MILLION_USD`, and `AEGIS_AI_OUTPUT_COST_PER_MILLION_USD` in the local shell. Values are never printed or committed. Locator advisory uses a bounded 2,000-token generation allowance for hidden reasoning plus final structured output, a 30-second timeout, at most one retry, and the existing per-request cost guard. Callers may select a smaller timeout; they cannot exceed the feature cap. The final schema remains narrow and its summary remains limited to 500 characters. Unit tests, validation, doctor, and CI never select live mode.
...
Rich interactive progress defaults to the `thinking` style: the symbol stays fixed while the message cycles through dim, normal, bright, normal, dim, normal at 150 ms intervals. ANSI blink is deliberately prohibited. Progress starts after 400 ms, so fast commands correctly show no animation. Actual stdout/stderr stream capabilities (`isTTY`, `getColorDepth()`, and `hasColors(16)`) are authoritative; terminal-brand variables such as `WT_SESSION` are only hints. A capable interactive Win32 stream can therefore auto-select `💭` even when brand variables are absent. `↻` is the non-emoji Unicode fallback and `[~]` is the ASCII fallback, and both retain thinking rather than rotating. `--progress-style=spinner` alone selects rotating frames, while `--progress-style=static` and `--no-animation` provide static stages. `--emoji`, `--no-emoji`, `--unicode`, and `--ascii` take precedence over auto-detection and control symbols independently of semantic color. Use `npm run cli:demo:progress -- --diagnose-terminal` for requested/effective modes, safe stream capabilities, and any fallback reason. The thinking symbol describes CLI work, not an AI call, and no presentation option changes evaluation results or authorizes healing.

See [CLI presentation experience](docs/cli-experience.md) for the complete progress, fallback, accessibility, and machine-output contracts.
