...
Rich interactive progress defaults to the `thinking` style: the symbol stays fixed while the message cycles through dim, normal, bright, normal, dim, normal at 150 ms intervals. ANSI blink is deliberately prohibited. Progress starts after 400 ms, so fast commands correctly show no animation. Windows Terminal auto-selects `💭`; `↻` is the non-emoji Unicode fallback and `[~]` is the ASCII fallback. `--progress-style=spinner` retains rotating frames, while `--progress-style=static` and `--no-animation` provide static stages. `--emoji`, `--no-emoji`, `--unicode`, and `--ascii` control symbols independently of color. Use `npm run cli:demo:progress -- --diagnose-terminal` for a safe capability summary. The thinking symbol describes CLI work, not an AI call, and no presentation option changes evaluation results or authorizes healing.
